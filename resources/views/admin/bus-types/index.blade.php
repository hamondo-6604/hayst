        ...
        <table class="w-full text-left border-collapse">
            ...
            <tbody class="divide-y divide-slate-100 dark:divide-slate-700">
                ...
                <x-modal id="edit-bus-type-modal-{{ $type->id }}" title="Edit Bus Type" size="md">
                    <form action="{{ route('admin.bus-types.update', $type) }}" method="POST" onsubmit="handleAjaxForm(this, 'edit-bus-type-modal-{{ $type->id }}', () => window.location.reload())">
                        @csrf
                        @method('PUT')
                        <div class="space-y-6">
                            <div>
                            ...
                            <div>
                                ...
                                <textarea name="description" rows="2" class="w-full p-0 pl-3 pb-2 text-base bg-transparent border-0 border-b-2 border-slate-300 hover:border-slate-300 dark:border-slate-600 dark:hover:border-slate-600 text-slate-800 dark:text-white outline-none focus:outline-none focus:ring-0 focus:border-primary-500 transition-colors">{{ $type->description }}</textarea>
                            </div>
                        </div>
                        <x-slot:footer>
                ...
                <x-modal id="delete-bus-type-modal-{{ $type->id }}" title="Delete Bus Type" size="sm">
                    <form action="{{ route('admin.bus-types.destroy', $type) }}" method="POST" onsubmit="handleAjaxForm(this, 'delete-bus-type-modal-{{ $type->id }}', () => window.location.reload())">
                        @csrf
                        @method('DELETE')
...
<x-modal id="create-bus-type-modal" title="Add New Bus Type" size="md">
    <form action="{{ route('admin.bus-types.store') }}" method="POST" onsubmit="handleAjaxForm(this, 'create-bus-type-modal', () => window.location.reload())">
        @csrf
        <div class="space-y-6">
            <div>
                <label class="block text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-1">Type Name <span class="text-red-500">*</span></label>
                <input type="text" name="type_name" required placeholder="e.g. Economy, VIP Sleeper" class="w-full p-0 pl-3 pb-2 text-base bg-transparent border-0 border-b-2 border-slate-300 hover:border-slate-300 dark:border-slate-600 dark:hover:border-slate-600 text-slate-800 dark:text-white outline-none focus:outline-none focus:ring-0 focus:border-primary-500 transition-colors">
            </div>
        ...
        <x-slot:footer>
            ...
            <button type="submit" class="px-4 py-2 rounded-xl bg-primary-600 text-white font-semibold text-sm hover:bg-primary-700 transition-colors">Create Bus Type</button>
        </x-slot:footer>
    </form>
</x-modal>
@endsection
